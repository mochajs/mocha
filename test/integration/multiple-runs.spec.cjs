...
describe("multiple runs", function () {
  it("should be allowed to run multiple times if cleanReferences is turned off", function (done) {
    var path =
      require.resolve("./fixtures/multiple-runs/run-thrice.fixture.cjs");
  ...
  it("should not be allowed if the instance is disposed", function (done) {
    var path = require.resolve("./fixtures/multiple-runs/dispose.fixture.cjs");
    invokeNode(
  ...
  it("should not be allowed to run while a previous run is in progress", function (done) {
    var path =
      require.resolve("./fixtures/multiple-runs/start-second-run-if-previous-is-still-running.fixture.cjs");
    invokeNode(
      [path],
      function (err, res) {
        if (err) {
  ...
  it("should reset the hooks between runs", function (done) {
    var path =
      require.resolve("./fixtures/multiple-runs/multiple-runs-with-flaky-before-each.fixture.cjs");
    invokeNode([path], function (err, res) {
