...
reporter-options:
  ...
  it('should use config file reporter-options when no CLI option provided', function() {
    ...
    try {
      ...
      expect(options['reporter-option'], 'to equal', ['output=./test-reports/xunit.xml']);
    } finally {
  ...
  it('should override config file with CLI reporter-options', function() {
    const options = loadOptions(['--reporter-options', 'output=./test-reports/cli.xml']);
    expect(options['reporter-option'], 'to equal', ['output=./test-reports/cli.xml']);
  });

  it('should override config file with CLI reporter-options using = syntax', function() {
    const options = loadOptions(['--reporter-options=output=./test-reports/equal.xml']);
    expect(options['reporter-option'], 'to equal', ['output=./test-reports/equal.xml']);
  });

  it('should use the last CLI reporter-option when multiple are provided', function() {
    const options = loadOptions([
      '--reporter-options', 'first.xml',
      '--reporter-options', 'second.xml'
    ]);
    expect(options['reporter-option'], 'to equal', ['second.xml']);
  });

  it('should work with reporter-option alias', function() {
    const options = loadOptions(['--reporter-option', 'output=./test-reports/single.xml']);
    expect(options['reporter-option'], 'to equal', ['output=./test-reports/single.xml']);
  });
});
