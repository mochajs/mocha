...
const Hook = require("./hook.mjs").Hook;
var {
  assignNewMochaID,
  clamp,
...
class Suite extends EventEmitter {
  ...
  /**
   * Set or get fast `ms` or short-hand such as "100ms".
   *
  ...
  fast(ms) {
    if (!arguments.length) {
      return this._fast !== undefined
        ? this._fast
        : Math.floor(this.slow() / 2);
    }
    if (typeof ms === "string") {
      ms = milliseconds(ms);
    }
