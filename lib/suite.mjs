...
class Suite extends EventEmitter {
  ...
  clone() {
    ...
    suite.retryDelay(this.retryDelay());
   ...
   *
   * @description
   * Optional options.wait configures a delay (in ms, or ms shorthand string such as 500ms / 1s) applied before each retry attempt.
   * @private
   * @param {number|string} n
   * @param {{ wait?: number|string }} [options]
   * @return {Suite|number} for chaining
   */
  retries(n, options) {
    if (!arguments.length) {
    ...
    if (options && options.wait !== undefined) {
      this.retryDelay(options.wait);
    }
    return this;
  }

  /**
   * Set or get delay (in milliseconds) applied before each retry try
   * @private
   * @param {number|string} [ms]
   * @returns {Suite|number}
   */
  ...
  retryDelay(ms) {
    ...
    this._retryDelay = Math.max(0, parseInt(ms, 10) || 0);
    return this;
  }
  ...
  _createHook(title, fn) {
    ...
    hook.retryDelay(this.retryDelay());
    hook.slow(this.slow());
    hook.ctx = this.ctx;
    hook.file = this.file;
    return hook;
  ...
  addTest(test) {
    ...
    test.retryDelay(this.retryDelay());
    test.slow(this.slow());
    test.ctx = this.ctx;
