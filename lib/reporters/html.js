...
var statsTemplate = '<ul id="mocha-stats">'
  ...
  + '<li class="pendings"><a href="javascript:void(0);">pendings:</a> <em>0</em></li>'
...
function HTML(runner) {
  ...
  var pendings = items[2].getElementsByTagName('em')[0];
  var pendingsLink = items[2].getElementsByTagName('a')[0];
  ...
  // pending toggle
  on(pendingsLink, 'click', function() {
    unhide();
    var name = (/pending/).test(report.className) ? '' : ' pending';
    report.className = report.className.replace(/fail|pass|pending/g, '') + name;
    if (report.className.trim()) {
      hideSuitesWithout('test pending');
    }
  });
