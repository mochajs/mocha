...
exports.generateDiff = function (actual, expected) {
  ...
  return exports.inlineDiffs ? inlineDiff.print(diff) : unifiedDiff.print(diff);
};
...
function doDiff (expected, actual, inline) {
  return inline
    ...
    : convertDiff(diff.structuredPatch(null, null, expected, actual));
}

/**
 * Converts single line of `diff.structuredPatch` output to format that supports inline differencies.
 *
 * @api private
 * @param {string} line Line with first character with kind of changes and rest of payload
 * @return {Object} Line in format that supports inline differencies.
 */
function convertLine (line) {
  let kind = line[0];

  if (kind === '-' || kind === '+') {
    return { kind: kind, changes: [{ value: line.substr(1) }] };
  }
  return { kind: kind, value: line.substr(1) };
}

/**
 * Converts all lines of each hunk of `diff.structuredPatch` output to format that supports inline
 * differencies.
 *
 * @api private
 * @param {Object} diff Object with structure like to `diff.structuredPatch` output 
 * @return {Object} The supplied argument with converted `hunks[i].lines`.
 */
function convertDiff (diff) {
  for (var i = 0; i < diff.hunks.length; ++i) {
    var hunk = diff.hunks[i];
    hunk.lines = hunk.lines.map(convertLine);
  }
  return diff;
}
