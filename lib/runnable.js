...
class Runnable extends EventEmitter {
  ...
  /**
   * Whether this runnable can never time out: its own threshold is 0 or the
   * root's is which a spec file cannot override.
   *
   * @private
   * @returns {boolean}
   */
  _timeoutDisabled() {
    ...
    return Boolean(root) && root.timeout() === 0;
  }
  ...
  resetTimeout() {
    var self = this;
    // Still arm a timer when disabled so unref'd work can not exit early. Read issue #3817
    var ms = this._timeoutDisabled() ? MAX_TIMEOUT : this.timeout();

    this.clearTimeout();
    this.timer = setTimeout(function () {
      if (self._timeoutDisabled()) {
        return;
      }
  ...
  run(fn) {
    ...
    function done(err) {
      ...
      finished = true;
      if (!err && self.duration > ms && !self._timeoutDisabled()) {
