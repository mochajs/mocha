...
import { Base } from "../../reporters/base.js";
import { SerializableEvent, SerializableWorkerResult } from "../serializer.js";
...
const debug = debugFactory("mocha:reporters:buffered");
 ...
 * @public
 */
export const createParallelBufferedClass = ({
  Base: BaseImpl = Base,
  SerializableEvent: SerializableEventImpl = SerializableEvent,
  SerializableWorkerResult:
    SerializableWorkerResultImpl = SerializableWorkerResult,
} = {}) =>
  class ParallelBuffered extends BaseImpl {
    /**
    ...
    createListener(eventName) {
      const listener = (runnable, err) => {
        this.events.push(
          SerializableEventImpl.create(eventName, runnable, err),
        );
      };
     ...
     */
      ...
      callback(SerializableWorkerResultImpl.create(this.events, failures));
      this.events = []; // defensive
    }
  };

export const ParallelBuffered = createParallelBufferedClass();
export default ParallelBuffered;
