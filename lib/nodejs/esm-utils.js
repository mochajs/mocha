...
import { isMochaError } from "../errors.js";
import { createRequire } from "node:module";

const require = createRequire(import.meta.url);

const debug = dbg("mocha:esm-utils");
...
const formattedImport = async (file, esmDecorator = forward) => {
  if (path.isAbsolute(file)) {
    try {
      return await doImport(esmDecorator(url.pathToFileURL(file)));
    } catch (err) {
  ...
  return doImport(esmDecorator(file));
};

export const doImport = async (file) => import(file);
...
export const requireOrImport = process.features.require_module
  ? requireModule : tryImportAndRequire;
...
export const loadFilesAsync = async (
  ...
  for (const file of files) {
    preLoadFunc(file);
    const result = await requireOrImport(
      path.resolve(file),
      esmDecorator,
    );
