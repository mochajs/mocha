...
/**
 * Takes one arg of any type, returns a value of the same type as the arg
 * @typedef {<T>(arg: T) => T} Endomorphism
 */

/**
 * @param {string} file
 * @param {Endomorphism} esmDecorator
 * @returns
 */
...
/**
 * Utilizes Node.js' require(esm) feature to load ESM modules
...
const requireModule = async (file, esmDecorator) => {
  debug('requireModule(%s)', file);
  if (path.extname(file) === '.mjs') {
    debug('requireModule returning formattedImport');
    return formattedImport(file, esmDecorator);
  }
  let hasThrownRequireErr = false;
  try {
    debug('requireModule trying require(file)');
    return require(file);
  } catch (requireErr) {
    hasThrownRequireErr = true;
    debug('requireModule caught requireErr: %O', requireErr.message);
    try {
      debug('requireModule falling back');
      return dealWithExports(await formattedImport(file, esmDecorator));
    } catch (importErr) {
      debug(
        'requireModule caught importErr: [%s]: %O',
        importErr.code,
        importErr.message
      );
      ...
      if (importErr.code === 'ERR_INTERNAL_ASSERTION') {
        console.log(
          'requireModule importErr code === ERR_INTERNAL_ASSERTION, throwing importErr'
        );
        throw requireErr;
      }

      console.log('requireModule throwing importErr');
      throw importErr;
    }
  } finally {
    debug('requireModule exiting');
    debug('hasThrownRequireErr: %s', hasThrownRequireErr);
