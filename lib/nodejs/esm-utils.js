...
const requireModule = async (file, esmDecorator) => {
  ...
  try {
    ...
    try {
      ...
      // ERR_UNSUPPORTED_DIR_IMPORT means import() tried to resolve a directory
      // (ESM resolution) and failed, but require() may have a more informative
      // error (e.g., a TypeScript compilation error from ts-node).
