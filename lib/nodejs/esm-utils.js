...
const { isMochaError } = require("../errors.mjs");
...
const requireModule = async (file, esmDecorator) => {
  ...
  try {
    ...
    try {
      ...
      // When neither error clearly takes precedence, show both to the user
      // so they can diagnose the actual issue. The require() error often has
      // the root cause (e.g. ts-node transpile errors) while import() may
      // report a less helpful secondary error.
      // See: https://github.com/mochajs/mocha/issues/5792
      const combinedError = new Error(
        `Failed to load file via both require() and import().\n\n` +
        `[require() error]:\n${requireErr.message || requireErr}\n\n` +
        `[import() error]:\n${importErr.message || importErr}`
      );
      combinedError.code = 'ERR_MOCHA_MODULE_LOAD_FAILED';
      combinedError.cause = {requireErr, importErr};
      throw combinedError;
    }
  }
};
