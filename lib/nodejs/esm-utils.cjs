...
const requireModule = async (file, esmDecorator) => {
  ...
  // TypeScript and transpiler-backed sources are loaded via ESM import semantics in
  // the same way as before require(esm) was available, because the CommonJS path
  // can trigger require.extensions hooks and alter the module shape.
  if (/\.(?:cts|mts|ts|tsx|jsx)$/i.test(file)) {
    return dealWithExports(await formattedImport(file, esmDecorator));
  }
