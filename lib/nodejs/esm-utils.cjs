...
const { isMochaError } = require("../errors.js");
...
const requireModule = async (file, esmDecorator) => {
  ...
  try {
    ...
    try {
      ...
      // A directory specifier resolves under CommonJS but not under ESM so this error
      // only means the fallback import() never got far enough to say anything useful about the file.
      // Check this issue https://github.com/mochajs/mocha/issues/5792
      if (importErr.code === "ERR_UNSUPPORTED_DIR_IMPORT") {
        throw requireErr;
      }
...
exports.loadFilesAsync = async (
  ...
  for (const file of files) {
    const context = preLoadFunc(file);
    const result = await exports.requireOrImport(
      resolveFile(file),
      esmDecorator,
    );
    postLoadFunc(file, result, context);
  }
};
