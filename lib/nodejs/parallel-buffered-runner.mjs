...
import { createFatalError } from "../errors.mjs";
import utils from "../utils.js";
import { BufferedWorkerPool } from "./buffered-worker-pool.mjs";
...
<<<<<<<< HEAD:lib/nodejs/parallel-buffered-runner.cjs
...
const { createFatalError } = require("../errors.js");

const DEFAULT_WORKER_REPORTER =
  require.resolve("./reporters/parallel-buffered.cjs");
========
const { EVENT_RUN_BEGIN, EVENT_RUN_END } = Runner.constants;
const debug = debugFactory("mocha:parallel:parallel-buffered-runner");
const { setInterval, clearInterval } = global;
const { createMap, constants } = utils;
const { MOCHA_ID_PROP_NAME } = constants;

const DEFAULT_WORKER_REPORTER = fileURLToPath(
  new URL("./reporters/parallel-buffered.mjs", import.meta.url),
);
>>>>>>>> 8101ac0 (Convert worker pool modules to ESM):lib/nodejs/parallel-buffered-runner.mjs
 ...
 * @public
 */
export class ParallelBufferedRunner extends Runner {
  constructor(...args) {
    super(...args);
