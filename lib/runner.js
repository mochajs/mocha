...
Runner.prototype.runSuite = function(suite, fn){
  ...
  function next() {
    ...
    if (!curr) return done();
    runner = self.createRunner(suite);
    runner.run(next);
  }
...
Runner.prototype.createRunner = function(suite) {
  ...
  runner._globals = this._globals;
  this.wrapEvents(runner);
 ...
 * in parallel
 */
Runner.prototype.wrapEvents = function(runner) {
  var self = this;
  runner.eventCache = [];
  _events.forEach(function (event) {
    runner.on(event, function (data) {
      if (self.suite.parallel) {
        runner.eventCache.push({event: event, data: data});
...
Runner.prototype._rebroadcastAll = function (runner) {
  ...
  runner.eventCache.forEach(function (cache) {
    self.emit(cache.event, cache.data);
  });
};
...
Runner.prototype.run = function(fn){
  ...
  // run suites
  if (!this.suite.parent) this.emit('start');
  this.runSuite(this.suite, function(){
    debug('finished running');
    if (!this.suite.parent) self.emit('end');
  });
