...
const { constants: errorConstants } = require("./error-constants.mjs");
...
Runner.prototype.hook = function (name, fn) {
  ...
  function next(i) {
    ...
    hook._allowUncaught = self._effectiveAllowUncaught(hook);
...
/**
 * Resolve the effective allowUncaught flag for the given runnable by walking from the runnable through its ancestor suites and finally falling
 * back to the runner level flag. explicit false on a closer ancestor wins over one that is true on a more distant one.
 * @private
 * @param {Runnable} runnable
 * @returns {boolean}
 */
Runner.prototype._effectiveAllowUncaught = function (runnable) {
  function explicit(node) {
    if (!node) return undefined;
    if (node._allowUncaught !== undefined) return Boolean(node._allowUncaught);
    if (typeof node.allowUncaught === "boolean") return node.allowUncaught;
    return undefined;
  }

  var here = explicit(runnable);
  if (here !== undefined) return here;
  var node = runnable && runnable.parent;
  while (node) {
    var inherited = explicit(node);
    if (inherited !== undefined) return inherited;
    node = node.parent;
  }
  return Boolean(this.allowUncaught);
};
...
Runner.prototype.runTest = function (fn) {
  ...
  if (this._effectiveAllowUncaught(test)) {
    test._allowUncaught = true;
    return test.run(fn);
  }
  try {
    test.run(fn);
...
Runner.prototype._uncaught = function (err) {
  ...
  const allowUncaught = this.currentRunnable
    ? this._effectiveAllowUncaught(this.currentRunnable)
    : Boolean(this.allowUncaught);
  if (allowUncaught && !utils.isBrowser()) {
