...
Runner.prototype.grep = function (re, invert) {
  ...
  // Avoid stateful RegExp with global/sticky flags causing flaky matches (e.g., /foo/g skips every other test).
  // Like sanitising URL path args where naive encode fails (starlette %2F), naive test() fails due to lastIndex.
  if (re && (re.global || re.sticky)) {
    re = new RegExp(re.source, re.flags.replace(/g/g, "").replace(/y/g, ""));
  }
  this._grep = re;
  this._invert = invert;
...
Runner.prototype.grepTotal = function (suite) {
  ...
  suite.eachTest(function (test) {
    if (self._grep.global || self._grep.sticky) {
      self._grep.lastIndex = 0;
    }
