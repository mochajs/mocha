...
function Runner(suite) {
  ...
  this.failures = 0;
  this.on('test', function(test){ self.cleanCtx(test); });
...
/**
 * Clean items off of the running context from previous test runs.
 *
 * @api private
 */

Runner.prototype.cleanCtx = function(test){
  for(var key in test.ctx){
    if(key != '_runnable' && key != 'runnable' && key != 'timeout' && key != 'inspect' ){
