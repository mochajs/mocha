...
class Runner extends EventEmitter {
  ...
  constructor(suite, opts = {}) {
    ...
    this.failures = 0;
    // Failure counts snapshotted at test/suite start, to detect teardown cascades (#5925)
    this._failuresAtTestStart = 0;
    this._suiteFailuresAtStart = new WeakMap();
    this._failSecondary = false;
    /**
...
Runner.prototype.fail = function (test, err, force) {
  // Consume the secondary marker up front so it can't leak to a later failure (#5925)
  var secondary = this._failSecondary === true;
  this._failSecondary = false;
  ...
  // Mark after thrown2Error so the flag lands on the emitted error object (#5925)
  if (secondary) {
    err.secondary = true;
  }
  ...
  function next(i) {
    ...
    hook.run(function cbHookRun(err) {
      ...
      if (hook.pending) {
        ...
        // Teardown hook failing after an earlier failure in its flow is a cascade (#5925)
        if (
          (name === HOOK_TYPE_AFTER_EACH &&
            self.failures > self._failuresAtTestStart) ||
          (name === HOOK_TYPE_AFTER_ALL &&
            self.failures > (self._suiteFailuresAtStart.get(hook.parent) || 0))
        ) {
          self._failSecondary = true;
        }
        self.fail(hook, err);
...
Runner.prototype.runTests = function (suite, fn) {
  ...
  function next(err, errSuite) {
    ...
    // Snapshot failures at test start to detect afterEach cascades (#5925)
    self._failuresAtTestStart = self.failures;
...
Runner.prototype.runSuite = function (suite, fn) {
  ...
  // Snapshot failures at suite start to detect after-all cascades (#5925)
  this._suiteFailuresAtStart.set(suite, this.failures);
