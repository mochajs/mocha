...
Runner.prototype.run = function (fn, opts = {}) {
  ...
  // Override process.exit during test execution to prevent premature termination
  process.exit = function mochaOverriddenExit(code) {
    code = code !== undefined ? code : 0;
    debug("process.exit(%d) intercepted during test execution", code);
    var err = createProcessExitError(code);

    // Throw the error so it flows through Mocha's normal error handling.
    // For sync tests: caught by Runnable.run's try/catch → fail test.
    // For async tests: becomes uncaughtException → Runner._uncaught → fail test.
    throw err;
  };

  // Restore original process.exit on every run-end path, including the
  // early emit in Runner#_uncaught that bypasses `end()` below.
  this.prependOnceListener(constants.EVENT_RUN_END, function () {
