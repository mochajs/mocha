...
/**
 * Creates an error object to be thrown when process.exit() is called during test execution.
 * @static
 * @public
 * @param {number} code - The exit code passed to process.exit()
 * @returns {Error} Error with code {@link constants.PROCESS_EXIT_DURING_TEST}
 */
function createProcessExitError(code) {
  var err = new Error(
    `process.exit(${code}) was called during test execution. ` +
    'Test code should not call process.exit(). ' +
    'Failing this test.'
  );
  err.code = constants.PROCESS_EXIT_DURING_TEST;
  err.exitCode = code;
  return err;
}
