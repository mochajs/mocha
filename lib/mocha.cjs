...
function loadRootHooksFromRequires(mocha, requires = []) {
  utils.castArray(requires).forEach(function (mod) {
    let modpath = mod;
    ...
    const rootHooks =
      ...
      (requiredModule.mochaHooks ||
        (requiredModule.default && requiredModule.default.mochaHooks));

    if (rootHooks) {
      mocha.rootHooks(typeof rootHooks === "function" ? rootHooks() : rootHooks);
      debug("loaded root hooks from required module %s", modpath);
    }
  });
}
...
function Mocha(options = {}) {
  ...
  } else if (options.require && !options._requiresHandled) {
    loadRootHooksFromRequires(this, options.require);
  }
  /**
