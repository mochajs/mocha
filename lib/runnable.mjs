...
class Runnable extends EventEmitter {
  ...
  /**
   * Enable or disable the allowUncaught flag for this Runnable.
   * @description
   * Called with no args inside a test or hook to opt that single runnable into the --allow-uncaught behavior, uncaught exceptions thrown by the runnable will then bubble out of Mocha instead of being absorbed.
   * Passing false explicitly opts out and therefore overriding a parent suite or runner level setting.
   * @public
    ...
    if (this.async) {
      ...
      try {
        ...
        if (err instanceof PendingError) {
          ...
          this._allowUncaught ||
          (typeof this.allowUncaught === "boolean" && this.allowUncaught)
        ) {
          throw err;
        }
