...
class Runnable extends EventEmitter {
   ...
   * @private
   * @param {number} [n] - Number of retries
  ...
  /**
   * Set or get delay (milliseconds) applied before each retry attempt.
   * @private
   * @param {number|string} [ms]
   * @returns {Runnable|number}
   */
  retryDelay(ms) {
    if (!arguments.length) {
    ...
    this._retryDelay = Math.max(0, ms | 0);
    return this;
  }
