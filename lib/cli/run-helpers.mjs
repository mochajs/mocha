...
import { watchRun, watchParallelRun } from "./watch-run.mjs";
import collectFiles from "./collect-files.mjs";
import errors from "../errors.js";
import esmUtils from "../nodejs/esm-utils.js";
import PluginLoader from "../plugin-loader.js";

const debug = debugFactory("mocha:cli:run:helpers");
const { createInvalidLegacyPluginError } = errors;
const { requireOrImport } = esmUtils;
 ...
 * @private
 */
export const handleRequires = async (
  requires = [],
  { ignoredPlugins = [] } = {},
) => {
 ...
 * @returns {Promise<Runner>}
 */
export const runMocha = async (mocha, options) => {
  const {
    watch = false,
    extension = [],
    ignore = [],
 ...
 * @private
 */
export const validateLegacyPlugin = (opts, pluginType, map = {}) => {
  /**
