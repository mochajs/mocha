...
const { lookupFiles } = require("./lookup-files.js");
  ...
  if (!files.length) {
    ...
    const noneFoundMsg =
      unmatchedSpecFiles.length === 1
        ? `No test files found: ${JSON.stringify(
            unmatchedSpecFiles[0].pattern,
          )}` // stringify to print escaped characters raw
        : "No test files found";

    if (allowEmpty) {
      console.warn(pc.yellow(`Warning: ${noneFoundMsg}`));
      return { files: [], unmatchedFiles: [] };
    }

    console.error(pc.red(`Error: ${noneFoundMsg}`));
    process.exit(1);
  } else {
