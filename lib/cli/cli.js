...
exports.main = (argv = process.argv.slice(2), mochaArgs) => {
  ...
  yargs()
    ...
    .fail((msg, err, yargs) => {
      ...
      // we still can't know where the issue is. there is no information on any of these vars
      // but we can at least tell what type of error we are facing
      // in our case the null was due to: TypeError: Cannot convert object to primitive value
      console.error(`\n${symbols.error} ${ansi.red('ERROR:')} ${msg} ${err}`);
      process.exitCode = 1;
    })
