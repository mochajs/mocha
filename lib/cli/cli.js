...
var interfaceNames = Object.keys(require('../interfaces'));
...
// options
  ...
  var parseOptions = program.parseOptions;
  var opts = program.opts;

  // override the default parseOptions command to handle mocha options correctly
    // --opts, --no-colour
    // by default parseOptions is not enumerable, but we are extra safe here.
    // if you overwrite this then you probably made a mistake.
  Object.defineProperty(program, 'parseOptions', {
    configurable: false,
    enumerable: false,
    writable: false,
    value: function (argv) {
      // to mutate or not to mutate? decided not to, as it leads to more reliability
      var args = parser.expandArgs(parser.expandOpts(argv.slice()));

      // make sure rawAwrgs is updated
      if (program.rawArgs) {
        program.rawArgs = program.rawArgs.slice(0, 2).concat(args);
      } else {
        // if rawArgs isn't set parseOptions wont normally set it, so we don't either
      }

      var parsed = parseOptions.call(program, args);

      // fix things like the weirdness between --colors and --no-colours
      fixConfig(program, args);

      // reset opts function to what it should be again.
      // its better to override it before and after parsing
      // then to prevent it from being writable which could introduce errors later on.
      Object.defineProperty(program, 'opts', {
        configurable: true,
        enumerable: false,
        writable: true,
        value: opts
  ...
  program
    ...
    .option('-r, --require <name>', 'require the given module', collectRequires, [])
    ...
    .option('--globals <names>', 'allow the given comma-delimited global [names]', collectGlobals, [])
  ...
  // re-set opts(). help will still output --opts.
  // if you really want you can override opts() again. but you shouldn't
  Object.defineProperty(program, 'opts', {
    configurable: true,
    enumerable: false,
    writable: true,
...
/**
 * Helper to collect globals
 * @param {* } g
 * @param {*[]} globals
 * @return {*[]}
 */
function collectGlobals (g, globals) {
  return globals.concat(list(g));
}

/**
 * Helper to collect menu items
 * @param {* } mod
 * @param {*[]} requires
 * @return {*[]}
 */
function collectRequires (mod, requires) {
...
/**
 * Loads the opts file at the path and configures mocha accordingly
 * @param {string} file path and name of opts file (test/mocha.opts)
 * @param {Mocha} mocha
 */
exports.loadOptsFile = function (file, mocha) {
  var program = exports.makeCommand();

  program.parseOptions(['--opts', file]);
  exports.setMochaOptions(mocha, program);
};
 ...
 * @param {Command} program
 * @param {string[]} argv rawArgs
 */
function fixConfig (program, argv) {
  // --no-colors
  if (!program.colors) {
  ...
  // --colors
  if (~argv.indexOf('--colors') || ~argv.indexOf('-c')) {
    program.colors = true;
  }
}
