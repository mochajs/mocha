 ...
 *
 * <b>NOTE:</b> if makeCommand is used parser.expandOpts(argv) is not required
 *     and program.opts() will work as expected.
 *
 * @return {{program: Command, requires: string[], globals: *[]}}
 */
...
// options
  ...
  // override the default parse command to handle --opts correctly
 ...
 *
 * <b>NOTE: </b> will not expand --opts, either:
 * <ul>
 *     <li>use makeCommand to make the program command (and then it's handled correctly)</li>
 *     <li>call parser.expandOpts(argv) first, before cli.setMochaOptions</li>
 * </ul>
 *
 ...
 * @param {[]} globals
 */
exports.setMochaOptions = function (mocha, program, requires, globals) {
  requires = requires || [];
  globals = globals || [];
  var options;
  ...
  // --colors
  // todo move a level up
  if (~options.rawArgs.indexOf('--colors') || ~options.rawArgs.indexOf('-c')) {
