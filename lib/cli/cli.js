...
// If not already done, load mocha.opts
  // todo remove, should be loaded at a different point
...
// ##################################### options setting was here

// most of the action come from here
 ...
 */
function addActions (mocha, program) {
  program._name = 'mocha';
 ...
 * @param {[]} globals
 */
function setMochaOptions (mocha, options, requires, globals) {
  requires = requires || [];
  globals = globals || [];
 ...
 */
function run (mocha, options, files) {
  ...
  var purge;
  var rerun;
  var files = resolveFiles(program.args, program.opts()); // todo provide files to run command
 ...
 */
function resolveFiles (files, options) {
  var _files = files;
  var extensions = ['js'];
  ...
  _files.forEach(function (arg) {
    var newFiles;
    try {
      newFiles = utils.lookupFiles(arg, extensions, options.recursive || false);
    } catch (err) {
...
exports.setMochaOptions = setMochaOptions;
