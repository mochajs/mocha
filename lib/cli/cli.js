...
/**
 * Parses the arguments (handles opts expanding and fixing config discrepancies)
 * @param {Command} program
 * @param {string[]} argv
 * @return {{root, dir, base, ext, name}|*|number|Command}
 */
exports.parseArgv = function (program, argv) {
  var args = (!process.env.LOADED_MOCHA_OPTS) // prevent double expanding
    ? argv.slice(0, 2).concat(parser.expandOpts(argv.slice(2))) // dont need to expandArgs
    : argv.slice();
...
exports.loadOptsFile = function (file, mocha) {
  ...
  program.parseOptions(parser.expandOpts(['--opts', file]));
