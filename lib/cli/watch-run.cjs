...
// https://github.com/mochajs/mocha/issues/237
...
async function validatePathsAreWatched(watcher, dirPaths) {
  if (dirPaths.length === 0) {
    return;
  }
  const expectedWatchedPaths = dirPaths.map((dirPath) => path.dirname(dirPath));
  const maxAttempts = 400;
  const msBeforeRetrying = 5;
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    debug(`before .getWatched(): ${Date.now()}`);
    const watchedPaths = Object.keys(watcher.getWatched());
    debug(`after .getWatched(): ${Date.now()}`);
    if (
      expectedWatchedPaths.every((dirPath) => watchedPaths.includes(dirPath))
    ) {
      debug("validated that expected paths are watched");
      return;
    }
    await new Promise((resolve) => setTimeout(resolve, msBeforeRetrying));
  }
  debug("gave up waiting for watchers on %o", expectedWatchedPaths);
}
...
const createWatcher = (
  ...
  watcher.on("ready", async () => {
    debug("watcher reports ready");
    // chokidar sometimes reports "ready" prematurely
    await validatePathsAreWatched(watcher, missingPaths);
...
const createRerunner = (mocha, watcher, { beforeRun, clearScreen } = {}) => {
  ...
  const run = () => {
    try {
      ...
      mocha._initialGlobals = initialGlobals;
      runner = mocha.run(() => {
      ...
      initialGlobals = runner._initialGlobals;
    } catch (err) {
