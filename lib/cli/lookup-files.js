...
module.exports = function lookupFiles(
  ...
  if (!fs.existsSync(filepath)) {
    let pattern;
    const posixFilepath = filepath.replaceAll(path.win32.sep, path.posix.sep);
    if (isDynamicPattern(posixFilepath)) {
      // Handle glob as is without extensions
      pattern = posixFilepath;
    } else {
      // glob pattern e.g. 'filepath+(.js|.ts)'
      const strExtensions = extensions
        .map((ext) => (ext.startsWith(".") ? ext : `.${ext}`))
        .join("|");
      pattern = `${convertPathToPattern(posixFilepath)}+(${strExtensions})`;
