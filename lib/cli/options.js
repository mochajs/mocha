  ...
  if (args["reporter-option"] && Array.isArray(args["reporter-option"])) {
    ...
    const configOptions = args["reporter-option"].reduce((acc, opt) => {
      if (typeof opt === "string") {
        // Validate the format - should be exactly one '='
    ...
    for (let i = 0; i < originalArgs.length; i++) {
      ...
      if (arg === "--reporter-option" || arg === "--reporter-options") {
        ...
        arg.startsWith("--reporter-option=") ||
        arg.startsWith("--reporter-options=")
      ) {
        const opt = arg.substring(arg.indexOf("=") + 1);
        const parts = opt.split("=");
        if (parts.length !== 2) {
          throw createInvalidArgumentValueError(
            `invalid reporter option '${opt}'`,
    ...
    args["reporter-option"] = Object.entries(mergedOptions).map(
      ([key, value]) => `${key}=${value}`,
    );
  }
