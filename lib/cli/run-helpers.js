...
const exitMochaLater = code => {
  console.warn(
    `cli/run-helpers.js::exitMochaLater: entering with code ${code} (# of failures).`
  );
  debugger;
  // TODO: is there a better way to determin posix-exit-codes cli option here?
  const usePosixExitCodes = process.argv.includes('--posix-exit-codes');
  console.warn(
    `cli/run-helpers.js::exitMochaLater: use posix exit codes? ${usePosixExitCodes}`
  );
  const exitCode =
...
const exitMocha = code => {
  console.warn(`cli/run-helpers.js::exitMocha: entering with code ${code}`);
  // TODO: is there a better way to determin posix-exit-codes cli option here?
  const usePosixExitCodes = process.argv.includes('--posix-exit-codes');
  const clampedCode =
    usePosixExitCodes && code > 0 ? EXIT_FAILURE : Math.min(code, 255);
