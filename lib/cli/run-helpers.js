 ...
 * Sorts any require-defined root hooks into buckets to assign to the root suite
 * upon `Mocha` instantiation.
 * @param {string[]} requires - Modules to require
 * @returns {import('../mocha').MochaRootHooks} Any root hooks
 * @private
 */
exports.handleRequires = (requires = []) =>
  requires.reduce(
    (acc, mod) => {
      ...
      if (
        type(requiredModule) === 'object' &&
        type(requiredModule.mochaHooks) === 'object'
      ) {
        debug('found root hooks in required file %s', mod);
        const rootHooks = requiredModule.mochaHooks;
        acc.beforeAll = acc.beforeAll.concat(rootHooks.beforeAll || []);
        acc.beforeEach = acc.beforeEach.concat(rootHooks.beforeEach || []);
        acc.afterAll = acc.afterAll.concat(rootHooks.afterAll || []);
        acc.afterEach = acc.afterEach.concat(rootHooks.afterEach || []);
      }
 ...
 * @param {Object} fileCollectParams - Parameters that control test
 *   file collection. See `lib/cli/collect-files.js`.
 * @returns {Promise<import('../buffered-runner')>}
 * @ignore
 * @private
 */
 ...
 * @param {Mocha} mocha - Mocha instance
 * @param {Mocha.MochaOptions} opts - Command line options
 * @private
 * @returns {Promise<import('../runner').Runner>}
 */
