...
const createWatcher = (
  ...
  const _trace = (...a) =>
    console.error(`[TRACE-watch ${process.hrtime.bigint()}]`, ...a);
  _trace(
  ...
  watcher.on("ready", async () => {
    debug("watcher ready");
    _trace(
      "'ready' fired; getWatched=",
      JSON.stringify(watcher.getWatched()),
    );
  ...
  watcher.on("all", (_event, filePath) => {
    ...
    _trace(`'all': event=${_event} path=${filePath} allow=${allow}`);
    if (allow) {
...
const createRerunner = (mocha, watcher, { beforeRun } = {}) => {
  ...
  const _rtrace = (...a) =>
    console.error(`[TRACE-rerun ${process.hrtime.bigint()}]`, ...a);

  const run = () => {
    try {
      _rtrace("run() entered; calling mocha.run()");
      ...
      runner = mocha.run(() => {
        ...
        blastCache(watcher);
        _rtrace(
          `mocha.run cb; failures=${finishedRunner ? finishedRunner.failures : "?"}; connected=${process.connected}`,
        );
        if (typeof process.send === "function" && process.connected) {
          try {
            ...
            _rtrace("IPC runFinished sent");
          } catch (err) {
            _rtrace("IPC send threw:", err && err.message);
          }
        }
        if (rerunScheduled) {
          _rtrace("rerunScheduled -> rerun()");
          rerun();
        } else {
  ...
  const scheduleRun = () => {
    _rtrace(
      `scheduleRun: runner=${runner ? "running" : "null"} rerunScheduled=${rerunScheduled}`,
    );
    if (rerunScheduled) {
      return;
    }
