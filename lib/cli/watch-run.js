...
const TEST_MODE = Boolean(process.env.MOCHA_TEST_WATCH);
...
const createWatcher = (
  ...
  if (TEST_MODE) {
    ...
    globalSetupBarrier = new Promise(resolve => (resolveGlobalSetup = resolve));
  ...
  watcher.on('ready', () => {
    debug('watcher ready');
    ready = true;
    if (TEST_MODE) process.send({received: ['ready']});
  });

  watcher.on('all', function handleEvent(event, filePath, stat) {
    if (TEST_MODE) process.send({received: ['all', event, filePath]});
    if (!matcher.allow(filePath)) return;
  ...
  if (process.env.MOCHA_TEST_WATCH) {
...
const createRerunner = (mocha, watcher, matcher, {beforeRun} = {}) => {
  ...
  const run = () => {
    try {
      if (TEST_MODE) process.send({run: true});
      mocha = beforeRun ? beforeRun({mocha, watcher}) || mocha : mocha;
      runner = mocha.run(() => {
        debug('finished watch run');
        if (TEST_MODE) process.send({runFinished: true});
        runner = null;
        blastCache(matcher);
        if (rerunScheduled) {
          rerun();
