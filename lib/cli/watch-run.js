...
const createWatcher = (
  ...
  watcher.on("all", function handleEvent(event, filePath, stat) {
    ...
    function doHandle(event, filePath, stat) {
      debug(
        "doHandle [event: %s] [filePath: %s] [stat.birthtime: %d] [stat.mtime: %d] [stat.ctime: %d] [ready: %s] [globalFixtureContext: %s] [exiting: %s]",
        event,
        filePath,
        stat?.birthtime,
        stat?.mtime,
      ...
      if (
        ...
        (stat
          ...
          ? Math.max(stat.mtime, stat.ctime, stat.birthtime) > startTime
          : ready)
      ) {
