...
const createWatcher = (
  ...
  const rerunner = createRerunner(mocha, watcher, tracker, {
    beforeRun
  });
 ...
 * @param {GlobFilesTracker} tracker - GlobFilesTracker instance
 * @param {Object} [opts] - Options!
 ...
 * @private
 */
const createRerunner = (mocha, watcher, tracker, {beforeRun} = {}) => {
 ...
 * @private
 */
const blastCache = tracker => {
  const files = tracker.watchFilesSet;
  files.forEach(file => {
