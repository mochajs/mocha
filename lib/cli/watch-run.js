...
function createPathFilter(globPaths, basePath) {
  ...
  const patterns = globPaths.flatMap((globPath) => {
    return braces.expand(globPath);
  });

  // each pattern will have its own path
  for (const pattern of patterns) {
    const scan = pm.scan(pattern, { literalBrackets: false });
    debug("processing glob pattern: %s", pattern);

    // match `cleanPath` (path without the glob part) and its subdirectories
    const cleanPath = path.resolve(basePath, scan.base);
    ...
    const absPath = path.resolve(basePath, pattern);
    debug("absolute path: %s", absPath);
    (scan.isGlob ? res.match.globs : res.match.paths).add(absPath);
...
function matchPattern(filePath, pattern, matchParent) {
  ...
  for (const globPath of pattern.globs) {
    if (pm.makeRe(globPath, { dot: true, windows: true, literalBrackets: false }).test(filePath)) {
      return true;
    }
  }
