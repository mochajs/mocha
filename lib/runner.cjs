...
var runnableContext = require("./nodejs/runnable-context.cjs");
...
class Runner extends EventEmitter {
  ...
  constructor(suite, opts = {}) {
    ...
    this.suite = suite;
    this._opts = opts;
    this._runnableContext = runnableContext.createRunnableContext
      ? runnableContext.createRunnableContext()
      : null;
...
Runner.prototype.hook = function (name, fn) {
  ...
  function next(i) {
    ...
    self._withRunnableContext(hook, function () {
      hook.run(cbHookRun);
    });

    function cbHookRun(err) {
...
/**
 * Run "fn" with "runnable" registered as the owner of the current async
 * context so async work it schedules can be traced back to it. Falls back to
 * invoking fn directly where attribution is unavailable (e.g when the browser
 * bundle, where "runnable-context" is stubbed out).
 *
 * @private
 ...
 */
Runner.prototype._withRunnableContext = function (runnable, fn) {
  return this._runnableContext ? this._runnableContext.run(runnable, fn) : fn();
};

/**
 * @private
 * @returns {Runnable|undefined} the Runnable that owns the current async
 * context or undefined when attribution is unavailable or the code is
 * running outside of any tracked Runnable.
 */
Runner.prototype._owningRunnable = function () {
  return this._runnableContext ? this._runnableContext.getOwner() : undefined;
};
...
Runner.prototype.runTest = function (fn) {
  ...
  if (this.allowUncaught) {
    ...
    return self._withRunnableContext(test, function () {
      return test.run(fn);
    });
  }
  try {
    self._withRunnableContext(test, function () {
      test.run(fn);
    });
  } catch (err) {
    fn(err);
...
Runner.prototype._uncaught = function (err) {
  ...
  var owner = this._owningRunnable();
  var runnable = owner || this.currentRunnable;
  ...
  // A Runnable that already passed has now failed asynchronously. Record the
  // failure against it. when we know which Runnable owned the async work the
  // remaining tests are unaffected so keep running them. Without that
  // knowledge a later stray error could be misattributed to an unrelated test,
  // so, as Mocha has done through out all the times, bail out instead.
  if (runnable.isPassed()) {
    ...
    if (!owner) {
      ...
      debug("uncaught(): attributed to an already-passed Runnable; continuing");
    }
  } else {
