...
Runner.prototype._uncaught = function (err) {
  ...
  // Fail an already-passed Runnable that threw asynchronously, but
  // keep running remaining tests (#5251).
  if (runnable.isPassed()) {
    debug("uncaught(): Runnable has already passed; failing and continuing");
    this.fail(runnable, err);
  } else {
