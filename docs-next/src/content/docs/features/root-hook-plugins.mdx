...
In some cases, you may want a [hook](/next/features/hooks) before (or after) every test in every file.
These are called _root hooks_.
Previous to v8.0.0, the way to accomplish this was to use `--file` combined with root hooks (see _[Root Hooks Are Not Global](/next/features/parallel-mode#root-hooks-are-not-global)_).
...
A _Root Hook Plugin_ is a JavaScript file loaded via [`--require`](/next/running/cli#--require-module--r-module) which "registers" one or more root hooks to be used across all test files.

In browsers you can set root hooks directly via a `rootHooks` object: `mocha.setup({ rootHooks: { beforeEach() {...} } })`, see [`mocha.setup()`](/next/running/browsers)
...
:::tip
If you need to ensure code runs once and only once in any mode, use [global fixtures](/next/features/global-fixtures).
:::
...
1. Use `--require test/hooks.js` (even better: use a [config file](/next/running/configuring) with `{"require": "test/hooks.js"}`) when running your tests.
...
Running `mocha --require test/hooks.js test/test.spec.js` will run as before (and is now ready to be used with [`--parallel`](/next/running/cli#--parallel--p)).
