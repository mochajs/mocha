...
Browser Mocha supports many, but not all [CLI options](/next/running/configuring).
To use a CLI option that contains a "-", please convert the option to camelCase, (eg. `check-leaks` to `checkLeaks`).

### Options That Differ Slightly From [CLI Options](/next/running/configuring)
...
It is possible to use [built-in reporters](/next/reporters/spec) as well.
