...
See [Delayed Root Suite](/next/features/hooks#delayed-root-suite).
...
- Use [`.only`](/next/declaring/exclusive-tests) until you find the test that causes Mocha to hang
...
Read more about [running tests in parallel](/next/features/parallel-mode).
:::
...
Specify an explicit path to a [configuration file](/next/running/configuring).
...
Please use [configuration file](/next/running/configuring) instead.
:::
...
Specify an explicit path to a [`package.json` file](/next/running/configuring) (ostensibly containing configuration in a `mocha` property).
...
:::caution
`--file` is incompatible with [parallel mode](/next/features/parallel-mode).
:::
...
If you want to set hooks to run, e.g., before each test, use a [Root Hook Plugin](/next/features/root-hook-plugins).
:::

:::note
As of v8.0.0, Mocha supports `--require` for [NodeJS native ESM](/next/explainers/nodejs-native-esm-support).
...
:::caution
`--sort` is incompatible with [parallel mode](/next/features/parallel-mode).
:::
...
`node` flags can be defined in Mocha's [configuration](/next/running/configuring).
...
[V8](https://v8.dev/) flags can be defined in Mocha's [configuration](/next/running/configuring).
