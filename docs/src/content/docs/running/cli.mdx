...
You can also opt in on a per suite or per test basis by calling this.allowUncaught() from inside a describe block or a hook or an "it" callback. pass "false" to explicitly opt out and override a parent suite or runner level setting.
