...
{/* This output is defined in lib/cli/cli.js and lib/cli/run-option-metadata.cjs */}
...
Commands:
  init <path>  create a client-side Mocha setup at <path>
...
Options:
  ...
  --node-option, -n                  Node or V8 option (no leading "--")
  --reporter-option, --reporter-options, -O Reporter-specific options (<k=v,[k1=v1,..]>)
  --require, -r                      Require module
  --watch-files                      List of paths or globs to watch
  --watch-ignore                     List of paths or globs to exclude from watching
  --allow-uncaught                   Allow uncaught errors to propagate
  --async-only, -A                   Require all tests to use a callback (async) or return a Promise
  ...
  --fail-hook-affected-tests         Report tests as failed when affected by hook failures (before/beforeEach)
  ...
  --no-colors, -C                    Force-disable color output
  ...
  --jobs, -j                         Number of concurrent jobs for --parallel; use 1 to run in serial
...
With [`--watch`](#--watch--w), every run is checked against the global variables that existed before the first run.
Mocha never removes a leaked variable, so a leak keeps being reported until you restart Mocha even after you fix the test that caused it.

### `--compilers`

:::note
`--compilers` was removed in v6.0.0.
See [Compilers deprecation](/explainers/compilers-deprecation) for workarounds.
:::
...
- See the [Node.js guide to debugging](https://nodejs.org/en/learn/getting-started/debugging)
...
Each boolean flag can be _negated_ by prepending `--no-` to the flag name.
...
If the [`--enable-source-maps`](https://nodejs.org/api/cli.html#--enable-source-maps) flag
