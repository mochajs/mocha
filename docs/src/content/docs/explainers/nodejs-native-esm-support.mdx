---
description: Writing your tests as ES modules instead of CommonJS.
...
- [Custom reporters](/reporters/third-party) and [custom interfaces](/interfaces/third-party) can be ES modules but Mocha loads them with `require` so they cannot use top-level `await`
