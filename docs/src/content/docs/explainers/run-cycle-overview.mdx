...
8. The selected interface loads the test files _in no particular order_. Most interfaces use an interface-specific `global` context, which is how, e.g., `describe()` ends up as a global in a test file. The [`require` interface](/interfaces/require) is the exception: it keeps the DSL off `globalThis`, and test files import it from `mocha` instead.
